...
def get_service(request: Request) -> CompanionApplicationService:
...
def status_payload(service):
    s = service.status()
    return {
        "interaction_state": s["interaction_state"].value,
        "elapsed_minutes": s["elapsed_minutes"],
        "focus_session": serialize(s["focus_session"]),
...
def read_status(service: Service):
    return status_payload(service)
...
def current_focus(service: Service):
    return status_payload(service)
...
def evaluate(body: EvaluateRequest, service: Service):
    try:
        d = service.evaluate(body.explicit_reminder_due)
        return {
            "allowed": d.allowed,
...
def reduce_frequency(body: ReduceFrequencyRequest, service: Service):
    try:
        service.reduce_frequency(body.additional_minutes)
        return status_payload(service)
    except (ValueError, InvalidTransitionError) as e:
        conflict(e)
...
def explanation(service: Service):
    try:
        facts, text = service.explanation()
        return {"message": text, "facts": serialize(facts)}
    except (ValueError, InvalidTransitionError) as e:
        conflict(e)


@router.get("/settings")
def read_settings(service: Service):
    return serialize(service.settings)


@router.patch("/settings")
def patch_settings(body: SettingsPatch, service: Service):
    try:
        return serialize(service.patch_settings(body.model_dump(exclude_unset=True)))
    except ValueError as e:
        conflict(e)


@router.get("/events")
def read_events(service: Service, limit: int = Query(default=100, ge=1, le=500)):
    return [serialize(e) for e in service.events(limit)]
