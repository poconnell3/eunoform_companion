...
class SQLiteSettingsRepository:
    ...
    def save(self, s: UserSettings) -> None:
        with self.db.connect() as c:
            c.execute(
                ...
                (
                    s.initial_nudge_minutes,
                    s.repeat_nudge_minutes,
                    s.after_dismiss_cooldown_minutes,
                    s.after_accept_cooldown_minutes,
                    s.after_irritation_cooldown_minutes,
                    s.quiet_default_minutes,
                    s.interaction_intensity.value,
                    s.visual_lead_in_seconds,
                    s.maximum_nudge_words,
                    int(s.wellness_nudges_enabled),
                    int(s.muted),
                ),
            )
    ...
    def get(self) -> UserSettings:
        with self.db.connect() as c:
            r = c.execute("SELECT * FROM settings WHERE singleton_id=1").fetchone()
        if not r:
            return UserSettings()
        return UserSettings(
            r["initial_nudge_minutes"],
            r["repeat_nudge_minutes"],
            r["after_dismiss_cooldown_minutes"],
            r["after_accept_cooldown_minutes"],
            r["after_irritation_cooldown_minutes"],
            r["quiet_default_minutes"],
            InteractionIntensity(r["interaction_intensity"]),
            r["visual_lead_in_seconds"],
            r["maximum_nudge_words"],
            bool(r["wellness_nudges_enabled"]),
            bool(r["muted"]),
        )
...
class SQLiteFocusSessionRepository:
        ...
        with self.db.connect() as c:
            c.execute(
                ...
                (
                    ...
                    s.status.value,
                    s.initial_nudge_at.isoformat() if s.initial_nudge_at else None,
                    s.last_nudge_at.isoformat() if s.last_nudge_at else None,
                    s.next_eligible_nudge_at.isoformat() if s.next_eligible_nudge_at else None,
                    int(s.wellness_nudges_enabled),
                ),
            )
    ...
    def get_active(self):
        with self.db.connect() as c:
            r = c.execute("SELECT * FROM focus_sessions WHERE status='active' LIMIT 1").fetchone()
        return self._from(r) if r else None

    def get(self, id):
        with self.db.connect() as c:
            r = c.execute("SELECT * FROM focus_sessions WHERE id=?", (id,)).fetchone()
        return self._from(r) if r else None
    ...
    def _from(r):
        return FocusSession(
            ...
            status=FocusSessionStatus(r["status"]),
            initial_nudge_at=_dt(r["initial_nudge_at"]),
            last_nudge_at=_dt(r["last_nudge_at"]),
            next_eligible_nudge_at=_dt(r["next_eligible_nudge_at"]),
            wellness_nudges_enabled=bool(r["wellness_nudges_enabled"]),
        )
...
class SQLiteNudgeRepository:
    ...
    def save(self, e):
        with self.db.connect() as c:
            c.execute(
                """INSERT INTO nudge_events VALUES (?,?,?,?,?,?,?,?,?,?)
    ...
    def get(self, id):
        with self.db.connect() as c:
            r = c.execute("SELECT * FROM nudge_events WHERE id=?", (id,)).fetchone()
        return self._from(r) if r else None

    def latest_for_session(self, sid):
        with self.db.connect() as c:
            r = c.execute(
                "SELECT * FROM nudge_events WHERE focus_session_id=? "
    ...
    def list_recent(self, limit=100):
        with self.db.connect() as c:
            rows = c.execute(
                "SELECT * FROM nudge_events ORDER BY created_at DESC LIMIT ?", (limit,)
            ).fetchall()
    ...
    def _from(r):
        return NudgeEvent(
            id=r["id"],
            focus_session_id=r["focus_session_id"],
            created_at=_dt(r["created_at"]),
            policy_reason=r["policy_reason"],
            threshold_minutes=r["threshold_minutes"],
            elapsed_minutes=r["elapsed_minutes"],
            interaction_intensity=InteractionIntensity(r["interaction_intensity"]),
            expression_name=r["expression_name"],
            gesture_name=r["gesture_name"],
            outcome=NudgeOutcome(r["outcome"]),
        )
...
class SQLiteDeferralRepository:
    ...
    def get_active(self, now: datetime, focus_session_id: str | None = None) -> Deferral | None:
        ...
        return (
            Deferral(
                id=r["id"],
                nudge_event_id=r["nudge_event_id"],
                created_at=_dt(r["created_at"]),
...
class SQLiteQuietIntervalRepository:
    ...
    def save(self, q):
        with self.db.connect() as c:
            c.execute(
                """INSERT INTO quiet_intervals VALUES (?,?,?,?,?)
                ON CONFLICT(id) DO UPDATE SET
    ...
    def get_active(self, now: datetime) -> QuietInterval | None:
        with self.db.connect() as c:
            r = c.execute(
                ...
                "AND started_at<=? AND ends_at>? ORDER BY ends_at DESC LIMIT 1",
