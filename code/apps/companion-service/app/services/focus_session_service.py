...
class FocusSessionService:
    ...
    def stop(self) -> FocusSession:
        ...
        s.ended_at = self._clock.now()
        s.status = FocusSessionStatus.ENDED
        self._repository.save(s)
        return s

    def elapsed_minutes(self) -> int:
        s = self._repository.get_active()
        return 0 if s is None else int(s.elapsed(self._clock.now()).total_seconds() // 60)
    ...
    def apply_cooldown(self, minutes: int) -> FocusSession:
        ...
        if s is None:
            raise ValueError("No active focus session exists.")
        s.next_eligible_nudge_at = self._clock.now() + timedelta(minutes=minutes)
        self._repository.save(s)
        return s
