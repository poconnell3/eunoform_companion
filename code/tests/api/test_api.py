...
def test_initial_status_is_idle(tmp_path):
    with client_for(tmp_path,FixedClock(START)) as c:
        r=c.get('/status'); assert r.status_code==200; assert r.json()['interaction_state']=='idle'

def test_start_and_duplicate_start(tmp_path):
    with client_for(tmp_path,FixedClock(START)) as c:
        assert c.post('/focus-sessions').status_code==201
        r=c.post('/focus-sessions'); assert r.status_code==409

def test_policy_blocks_before_threshold(tmp_path):
    clock=FixedClock(START)
    with client_for(tmp_path,clock) as c:
        c.post('/focus-sessions'); clock.set(START+timedelta(minutes=20))
        r=c.post('/interactions/evaluate',json={}); assert r.json()['allowed'] is False
...
def test_full_accept_flow(tmp_path):
    ...
    with client_for(tmp_path,clock) as c:
        ...
        r=c.post('/interactions/current/accept'); assert r.status_code==200
...
def test_ten_minute_deferral_is_persisted_and_blocks(tmp_path):
    ...
    with client_for(tmp_path,clock) as c:
        advance_to_nudge(c,clock)
        r=c.post('/interactions/current/defer',json={'minutes':10}); assert r.status_code==200
        ...
        r=c.post('/interactions/evaluate',json={}); assert r.status_code==200

def test_dismissal_enforces_cooldown(tmp_path):
    clock=FixedClock(START)
    with client_for(tmp_path,clock) as c:
        advance_to_nudge(c,clock); c.post('/interactions/current/dismiss')
        r=c.post('/interactions/evaluate',json={}); assert r.json()['reason']=='blocked_cooldown'

def test_quiet_mode_blocks_and_can_end(tmp_path):
    clock=FixedClock(START)
    with client_for(tmp_path,clock) as c:
        c.post('/focus-sessions')
        r=c.post('/interactions/current/quiet',json={'minutes':60}); assert r.json()['interaction_state']=='quiet'
        r=c.post('/interactions/current/quiet/end'); assert r.json()['interaction_state']=='focusing'

def test_explanation_uses_recorded_facts(tmp_path):
    clock=FixedClock(START)
    with client_for(tmp_path,clock) as c:
        advance_to_nudge(c,clock)
        r=c.get('/interactions/current/explanation'); assert r.status_code==200
...
def test_settings_patch_and_validation(tmp_path):
    with client_for(tmp_path,FixedClock(START)) as c:
        r=c.patch('/settings',json={'initial_nudge_minutes':30,'muted':True}); assert r.status_code==200
...
def test_events_endpoint_records_nudge(tmp_path):
    ...
    with client_for(tmp_path,clock) as c:
        advance_to_nudge(c,clock)
        events=c.get('/events').json(); assert len(events)==1; assert events[0]['policy_reason']=='allowed_wellness_threshold'

def test_invalid_accept_returns_conflict_without_state_change(tmp_path):
    with client_for(tmp_path,FixedClock(START)) as c:
        c.post('/focus-sessions'); r=c.post('/interactions/current/accept'); assert r.status_code==409
        assert c.get('/status').json()['interaction_state']=='focusing'

def test_active_session_survives_app_restart(tmp_path):
    db=tmp_path/'test.sqlite3'; clock=FixedClock(START)
    with TestClient(create_app(database_path=db,clock=clock)) as c: c.post('/focus-sessions')
    with TestClient(create_app(database_path=db,clock=clock)) as c: assert c.get('/status').json()['interaction_state']=='focusing'

def test_stop_focus_works_during_quiet(tmp_path):
    with client_for(tmp_path,FixedClock(START)) as c:
        c.post('/focus-sessions'); c.post('/interactions/current/quiet',json={'minutes':60})
        r=c.post('/focus-sessions/current/stop'); assert r.status_code==200; assert r.json()['interaction_state']=='idle'
